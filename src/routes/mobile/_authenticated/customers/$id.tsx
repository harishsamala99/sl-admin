...
function MobileCustomerDetail() {
  ...
  return (
    <div className="space-y-4 animate-fade-in pb-10">
      <Link
        to="/mobile/customers"
        className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground p-1 -ml-1"
      >
      ...
      <div className="bg-card border border-border/50 rounded-xl p-5 shadow-sm space-y-4 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            <Badge
              variant="outline"
              className={`capitalize font-medium ${statusColor[status] ?? statusColor.active}`}
            >
              {status}
            </Badge>
            {(customer.tags ?? []).map((t) => (
              <Badge
                key={t}
                variant="outline"
                className="bg-gold/10 text-gold border-gold/40 font-medium"
              >
                {t}
              </Badge>
            ))}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent
          value="info"
          className="space-y-4 mt-4 animate-in fade-in slide-in-from-bottom-2"
        >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {customer.card_last4 ? (
                    <div>
                      {customer.card_brand ?? "Card"} •••• {customer.card_last4}
                    </div>
                  ) : (
        ...
        <TabsContent
          value="history"
          className="space-y-3 mt-4 animate-in fade-in slide-in-from-bottom-2"
        >
          {bookings.length === 0 ? (
            ...
            bookings.map((b) => (
              <div
                ...
                className="bg-card border border-border/50 p-4 rounded-xl shadow-sm space-y-3 active:scale-[0.98] transition-transform"
              >
        ...
        >
          <div className="flex gap-2">
            ...
            <Button
              onClick={addNote}
              className="h-auto gradient-gold text-primary-foreground border-0 px-3"
            >
              Add
            </Button>
          </div>
          ...
          <div className="space-y-3">
            ...
            {notes.map((n) => (
              <div
                key={n.id}
                className="bg-card border border-border/50 p-3.5 rounded-xl shadow-sm"
              >
                <div className="text-sm whitespace-pre-wrap">{n.body}</div>
                <div className="text-[10px] text-muted-foreground mt-2 font-medium">
                  {formatDate(n.created_at)} at{" "}
